(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            ...
            ctx.moveTo(0, 0);
            ctx.scale(0.75, 0.75);
            ctx.font = "12px 微软雅黑,Verdana"; // Why isn't the font size working? (ˉ(∞)ˉ)
            ctx.fillText("Click mo babi", 23, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
    ...
    Bloom = function(tree, point, figure, color, alpha, angle, scale, place, speed) {
        this.tree = tree;
        this.point = point;
        this.color = color || 'rgb(' + random(128, 200) + ',' + random(0, 100) + ',' + random(180, 255) + ')';
